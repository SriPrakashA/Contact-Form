...
function ContactList() {
    ...
    const handleDelete = (val) => {
        var userResponse = window.confirm("Are you sure you want to delete this contact?");
        if (userResponse) {
    ...
    return (<>
        <div className="py-4 px-3 head" >
            ...
            <button type="button" className="btn btn-primary confirm-btn" onClick={handleAdd} >Add Contact</button>
        </div>
                    ...
                    {Array.isArray(data) &&
                        data.map((val, i) => (
                            <tr key={i} >
                                ...
                                <td>
                                    <i class="fa-regular fa-pen-to-square edit-icon" title="edit" onClick={() => handleEdit(val)}></i>
                                    <i class="fa-solid fa-circle-minus del-icon" title="delete" onClick={() => handleDelete(val)} ></i>
                                </td>
                            </tr>
                        ))
                    }
